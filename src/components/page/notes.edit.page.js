...
export class NotesEditPage extends React.Component {
    ...
    componentDidMount() {
        if (this.props.match.params.noteId){
            getNote(this.props.match.params.customerId, this.props.match.params.noteId).then(data => {
                this.setState({note: data, isLoaded: true, typeCreatePage: false, title: 'Edit Page'});
            })
        }else{
                    ...
                    <Formik
                        ...
                        onSubmit={(values) => {
                            ...
                            if (this.state.typeCreatePage){
                                createNote(this.props.match.params.customerId, values).then((result) => {
                                    console.log(result)
                                    if (result.ok == 1) {
                                ...
                                updateNote(this.props.match.params.customerId, this.props.match.params.noteId, values).then((result) => {
                                    console.log(result)
                                    if (result.ok == 1) {
                                        this.props.history.push('/customers')
                                    }

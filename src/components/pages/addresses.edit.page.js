...
export class AddressesEditPage extends React.Component {
  ...
  render() {
    if (!this.state.isLoaded) {
      ...
      this.state.isLoaded &&
      this.state.address.AddressLine === undefined
    ) {
      return (
        <>
          <h2>Address not found</h2>
          <Link
            to={`/customers/${this.props.match.params.customerId}/addresses/create`}
          >
            Create address
          </Link>
            ...
            {({ values, errors, touched, handleChange, handleSubmit }) => (
              <form onSubmit={handleSubmit}>
                ...
                <input
                  ...
                  onChange={handleChange}
                />
                {errors.SecondAddressLine &&
                  touched.SecondAddressLine &&
                  errors.SecondAddressLine}
                <input
                  name="AddressType"
                  type="string"
